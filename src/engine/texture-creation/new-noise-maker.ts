...
export class Svg {
  ...
  addTurbulence(seed: number, frequency: number | [number, number], octaves: number, noiseType = NoiseType.Turbulence) {
  ...
  addColorMatrix(matrix: number[]) {
    this.filterComponents.push(`<feColorMatrix color-interpolation-filters="sRGB" values="${matrix}"/>`);
    return this;
  }
